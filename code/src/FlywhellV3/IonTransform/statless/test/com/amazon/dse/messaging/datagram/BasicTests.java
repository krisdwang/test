...
public class BasicTests {
    ...
    public void testRandom() throws EncodingException, DecodingException {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            in.put("Round " + i, checker._mapSerializer.generateRandom(rand, 0));
        }
        ...
        for (int i = 0; i < 1000; i++) {
            Random rand = new Random(i);
            in = checker._mapSerializer.generateRandom(rand, 0);
            bout = conv.serialize(in);
            out = conv.deserialize(bout);
            assertTrue(checker.areEqual(in, out));
        }
    }
    
    @Test
    public void testEmpty() throws EncodingException, DecodingException {
        Map m = new HashMap();
    ...
    @Test
    public void testFooBar() throws EncodingException, DecodingException {
        Map m = new HashMap();
        m.put("Foo", "Bar");
        byte[] bout = conv.serialize(m);
        Map out = conv.deserialize(bout);
        ...
        if (ir.hasNext()) {
            ...
            while (ir.hasNext()) {
                ...
                assertEquals(ir.stringValue(), "Bar");
            }
        }
    ...
    @Test
    public void testBoolean() throws EncodingException, DecodingException {
        Map m = new HashMap();
        m.put("True", true);
        m.put("False", false);
        m.put("Null", null);
    ...
    @Test
    public void testDeserializeInputStreamContainingMultipleIonDatagrams()
    {
      try {
                      BufferDatagramConverter bdc = new IonDatagramConverter();
              ...
              byte[] bytes1 = bdc.serialize(map1);
              byte[] bytes2 = bdc.serialize(map2);
              
                      byte[] combo = new byte[bytes1.length+bytes2.length];
                      System.arraycopy(bytes1, 0, combo, 0, bytes1.length);
                      System.arraycopy(bytes2, 0, combo, bytes1.length, bytes2.length);
                      
                      InputStream is = new ByteArrayInputStream(combo);
                      Map<String, String> map = bdc.deserialize(is);
                      assertEquals("v11", map.get("key11"));
                      assertEquals("v12", map.get("key12"));
                      assertEquals(null, map.get("key13"));
                      
                      assertTrue(is.available()>0);
                      
                      map = bdc.deserialize(is);
                      assertEquals("v21", map.get("key21"));
                      assertEquals("v22", map.get("key22"));
                      assertEquals(null, map.get("key23"));
    ...
    public void testSampleMap() {
        ...
        assertEquals(ir.doubleValue(), 12242.124598129,0);
        assertTrue(ir.hasNext());
        ...
        assertEquals(ir.longValue(), (long) 9326);
        assertTrue(ir.hasNext());
        assertEquals(ir.next(), IonType.INT);
...
//        IonReader cursor = IonIterator.makeIterator(ir);
