...
public class AForkJoinTest {
    ...
    public void testA() {
        ...
        AbstractApplicationContext context = SpringUtil.getContext("config/beans-A-fj.xml");
        ...
        ForkJoin fj2 = (ForkJoin) p.getShutdown().iterator().next();

        context.close();

        NamedEvent acquireEvent = new NamedEvent(fj1, LiveEvent.acquire.toString());
        NamedEvent releaseEvent = new NamedEvent(fj2, LiveEvent.release.toString());
        ...
        assertTrue(tally.inSequence(
                new Notch(fj1, acquireEvent, "end"),
                new Notch(fj2, releaseEvent, "begin")));

        assertTrue(tally.inSequence(
                new Notch(fj2, releaseEvent, "begin"),
                new Notch(workerA, releaseEvent, "begin"),
                new Notch(workerA, releaseEvent, "end"),
                new Notch(fj2, releaseEvent, "end")));
        assertTrue(tally.inSequence(
                new Notch(fj2, releaseEvent, "begin"),
                new Notch(workerB, releaseEvent, "begin"),
                new Notch(workerB, releaseEvent, "end"),
                new Notch(fj2, releaseEvent, "end")));
